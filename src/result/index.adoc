...
The `Result<T>` type is a bifunctor that implements:

* the `Result.to(mapValue, mapError)` method as a `bimap` function.
* the `Result.to(mapValue)` method as a `second` function.
* the `Result.failure(mapError)` method as a `first` function.
...
+
+
4. Applying the `bimap` function is the same as applying `first` and `second` functions.
+
[source,typescript]
----
...
+
This method can be used to track all occurred failures,
