...
+
* `Result.to<U>(biMap: [(value: T) => U, (error: Error) => Error]): Result<U>`:
** For a `Success`,
applies the first callback of a given `biMap` pair to the `Success.value`
and returns its result wrapped as a `Success`.
** For a `Failure`,
applied the second callback of a given `biMap` pair to the `Failure.value`
...
function entityByIdRequest(id: string): Result<Request> {
    return validInput(id)
        .to(failureWith(chainedException('Entity ID {{id}} is invalid' { // <.>
            id,
        })))
        .to(apiRequest('GET'));
}
----
<.> You can also combine both callbacks into one `Result.to(apiRequest(...), chainedException(...))` call.


=== `Result.into()`

* `Result.into<U>(reduce: Unary<T | Error, U>): U`
...
* `Result.run(valueProcedure: UnaryVoid<T>, errorProcedure?: UnaryVoid<Error>): this`:
** For a `Success`, runs a given `valueProcedure` and returns itself.
** For a `Failure`, runs a given `errorProcedure` and returns itself.
+
* `Result.run(procedures: BiVoidResult<T>): this`:
** For a `Success`, runs the first procedure in the `procedures` pair and returns itself.
** For a `Failure`, runs the second procedure in the `procedures` pair and returns itself.
