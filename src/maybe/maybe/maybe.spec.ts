...
describe(Maybe, () => {
    ...
    describe('run', () => {
        ...
        it('must be assigned to Maybe', () => {
            const output: Maybe<number> = maybe(unsafe(pi)).run(assignPi(3.1415));
        ...
        it('cannot be assign to Just', () => {
            // @ts-expect-error -- TS2322: Type 'Maybe' is not assignable to type 'Just'.
            const output: Just<number> = maybe(unsafe(pi)).run(assignPi(3.1415));
        ...
        it('cannot be assigned to Nothing', () => {
            // @ts-expect-error -- TS2322: Type 'Maybe<number>' is not assignable to type 'Nothing<number>'.
            const output: Nothing<number> = maybe(unsafe(pi)).run(assignPi(3.1415));
    ...
    describe('lift', () => {
        it('does not accept functions with strictly-typed input', () => {
            ...
            //  Argument of type '(input: number) => string'
            //  s not assignable to
            //  parameter of type '(value: number | null | undefined) => string | null | undefined'.
            expect(maybe(unsafe(0)).lift(strictDecimalOutput))
                .toStrictEqual(just('0'));
        });

        it('must be assigned to Maybe', () => {
            const output: Maybe<string> = maybe(unsafe(0)).lift(unsafeDecimalOutput);
        ...
        it('cannot be assigned to Just', () => {
            // @ts-expect-error -- TS2322: Type 'Maybe<string>' is not assignable to type 'Just<string>'.
            const output: Just<string> = maybe(unsafe(0)).lift(unsafeDecimalOutput);
        ...
        it('cannot be assigned to Nothing', () => {
            ...
            const output: Nothing<string> = maybe(unsafe(null)).lift(unsafeDecimalOutput);
