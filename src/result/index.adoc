...
The `Result<T, E>` type is a monad that implements:
...
----
let ma: Result<T, E>;
let f: (value: T) => Success<U, E>;
let g: (value: U) => Success<V, E>;
...
* `Result#onto<U>(flatMap: (value: T) => Result<U, E>): Result<U, E>`:
...
then the `apiResponse` fails with an `Error`.
<.> When we have an invalid `id`,
then neither the `apiRequest` nor `apiResponse` callbacks are called.
+
So even it is a `HEAD` request,
