...
== Using the `into()` method with the `maybeOf()` function

The `Maybe.into()` method allows folding a Maybe instance into a different type.
It applies the argument function for present and absent values.
In combination with the `maybeOf()` function,
...
maybe(null).into(maybeOf(decimal)) == just(decimal(null)) // <.>
----
...
<.> Use the `maybeOf()` function as a shortcut.

[NOTE]
====
Since v0.9.0,
`Maybe.into(maybeOf)` replaced the `Maybe.lift(map)` method.
====
...
* `maybeOf<T, U>(map: Unary<T | null | undefined, U | null | undefined>): Unary<T | null | undefined, Maybe<U>>`
...
* `justOf<T, U>(map: Unary<T | null | undefined, Present<U>>): Unary<T | null | undefined, Just<U>>`
