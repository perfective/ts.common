...
----
<1> Inherited from `Error`, message is generated from the `template` and `context`.
<2> Message template with tokens wrapped in `{{ }}`.
<3> Record of context information.
Context data can be rendered in the template as part of the message.
<4> It's possible to provide a previous error.
<5> Human-readable output of all the errors.
...
which can be convenient for localization or handling purposes.
